...
o_path = os.path.abspath(os.path.join( os.pardir,os.pardir,'result', 'contagion', 'temporal', 'synthetic', 'model_1_0', 'out'))
if not os.path.exists(o_path ):
    os.makedirs( o_path)


# Path to save output and error file for bash job
e_path = os.path.abspath(os.path.join( os.pardir, os.pardir, 'result', 'contagion', 'temporal', 'synthetic', 'model_1_0', 'error'))
if not os.path.exists(e_path ):
    os.makedirs( e_path)

# Path to data  file 
d_path = os.path.abspath(os.path.join( os.pardir,os.pardir,'data', 'synthetic', 'model_1_0'))
if not os.path.exists(o_path ):
    raise ValueError

# Path to save output  file for result
r_path = os.path.abspath(os.path.join( '/projects/b1022/Projects/junelee/', 'movie', 'contagion','temporal', 'synthetic', 'model_1_0'))

if not os.path.exists(r_path ):
    os.makedirs(r_path)


programname = 'temporal_contagion.py'
...
for p, d, t, n in pdtn_list:
    ...
    with open(bash_path + "/job_script_synthetic_1_0_p{}d{}t{}n{}.sh".format(int(100*p), int(100*d), int(100*t), int(n)), 'w') as queue_out:
            queue_out.write(
"""
#!/bin/bash
#MSUB -N contagion_jobscript_real
#MSUB -A b1022
...
""".format(o_path, e_path, current_path, programname, d_path, r_path, belief_type, p, d, t, n))
    ...
    queue_out.close()
    os.system("msub {}/job_script_synthetic_1_0_p{}d{}t{}n{}.sh".format(bash_path, int(100*p), int(100*d), int(100*t), int(n)))
